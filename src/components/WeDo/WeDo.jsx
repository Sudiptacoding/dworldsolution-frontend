...
// আপনার ব্যাকএন্ড API এর URL (প্রয়োজনীয় পরিবর্তন করে নিতে পারেন)
const API_URL = "https://dworldsolution-backend.vercel.app/api/cards";

const MarqueeRow = ({ cards, direction }) => {
...
const WeDo = () => {
  ...
  useEffect(() => {
    const loadCards = async () => {
      try {
        ...
        console.error("Failed to load cards from backend:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="py-16 overflow-hidden bg-black text-white">
      {/* Heading */}
      <div className="relative z-10 max-w-3xl px-4 mx-auto mb-10 text-center">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
      ...
      {loading ? (
        <div className="text-center text-gray-400 font-syne">Loading...</div>
      ) : cards.length > 0 ? (
        <>
          {/* Marquee Rows */}
          <MarqueeRow cards={cards} direction="left" />
          <MarqueeRow cards={cards} direction="right" />
        </>
      ) : (
        <div className="text-center text-gray-500 font-syne">No core services found.</div>
      )}
    </div>
  );
};
