...
//       <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5 }}>
//         <div
//           className={`grid gap-6 max-w-5xl mx-auto px-4 items-start ${
//             activeCategory === "Shorts" || selectedType === "short"
//               ? "grid-cols-1 sm:grid-cols-2 md:grid-cols-3"
//               : "grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2"
//           }`}
...
//                 key={item.id || idx}
//                 className={`relative w-full overflow-hidden rounded-lg cursor-pointer group bg-neutral-900 border border-neutral-800 ${
//                   isShorts ? "max-w-[280px] sm:max-w-[320px] mx-auto" : ""
//                 }`}
//                 style={{ paddingTop: aspectRatio }}
//               >
//                 {playingIndex === idx && (item.type === "video" || item.type === "short") ? (
...
//                         {item?.type}
//                       </span>
//                       <h3 className="text-sm md:text-base font-semibold text-white drop-shadow-md">{item?.title}</h3>
//                       {/* Topics Tag List */}
//                       {item.topics && item.topics.length > 0 && (
//                         <div className="flex flex-wrap gap-1 mt-1 opacity-80">
//                           {item.topics.slice(0, 3).map((topic, tIdx) => (
...
//             className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90 p-4"
//           >
//             <button onClick={() => setLightboxImage(null)} className="absolute top-4 right-4 text-white hover:text-gray-300 z-50">
//               <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
//               </svg>
//             </button>
...
function WorkProjectSection() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        // যদি ইউআরএল-এ আগের কোনো ক্যাটাগরি সেট করা না থাকে, তবে প্রথম ক্যাটাগরিকে্যাক্টিভ করবে
        const initialCategory = searchParams.get("category");
        if (!initialCategory && cats.length > 0) {
          setActiveCategory(cats[0]);
        }
  ...
  return (
    <div className="pt-0 text-white bg-black md:pt-0 scroll-mt-10">
      {/* Heading */}
      <div className="px-4 pt-0 pb-12 text-center md:pt-0 md:pb-16 sm:px-6 lg:px-8">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.1 }}
      ...
      {/* Grid Content - Perfect Pinterest Masonry Layout */}
      <motion.div 
        initial={{ opacity: 0, y: 30 }} 
        whileInView={{ opacity: 1, y: 0 }} 
      ...
      >
        {(() => {
          // অরিজিনাল ইনডেক্স ট্র্যাক করার জন্য ম্যাপ করা হলো
          const itemsWithIndex = filteredItems?.map((item, idx) => ({ ...item, originalIdx: idx })) || [];
          const isThreeColumns = activeCategory === "Shorts" || selectedType === "short";
          const numCols = isThreeColumns ? 3 : 2;

          // খালি কলাম তৈরি করা
          const columns = Array.from({ length: numCols }, () => []);
          
          // আইটেমগুলোকে অল্টারনেট কলামে ভাগ করা (Masonry logic)
            ...
            >
              {columns.map((colItems, colIdx) => (
                <div key={colIdx} className="flex flex-col gap-6">
                  {/* {colItems.map((item) => {
                    // const isShorts = item?.type === "short" || item?.category === "Shorts";
                    // const aspectRatio = isShorts ? "177.77%" : "56.25%"; // Shorts 9:16, Videos 16:9
                    // const idx = item.originalIdx;
                    const isShorts = item?.type === "short" || item?.category === "Shorts";
                  
                  // আমরা কি শুধু শর্টস ক্যাটাগরি বা শর্টস ফরম্যাটে আছি?
                  const isPureShortsMode = activeCategory === "Shorts" || selectedType === "short";
                  
                  // কন্ডিশন: মিক্সড মোডে শর্টসের হাইট ১৪০% হবে, কিন্তু শুধু শর্টস মোডে ১৭৭.৭৭% ই থাকবে
                  const aspectRatio = isShorts 
                    ? (isPureShortsMode ? "177.77%" : "110%") 
                    : "56.25%";
                  ...
                  const idx = item.originalIdx;
                    ...
                    return (
                      <div 
                        key={item.id || idx} 
                        className={`relative w-full overflow-hidden rounded-lg cursor-pointer group bg-neutral-900 border border-neutral-800 transition-all duration-300 ${
                          isShorts ? "max-w-[280px] sm:max-w-[320px] mx-auto" : ""
                        }`} 
                        style={{ paddingTop: aspectRatio }}
                      > */}
              ...
              const aspectRatio = isShorts 
                ? (isPureShortsMode ? "177.77%" : "140%") 
                : "56.25%";
                
              const idx = item.originalIdx;

              // ২-কলামের গ্রিডে শর্টসগুলোর ডাইনামিক বর্ডার অ্যালাইনমেন্ট লজিক
              let alignmentClass = "mx-auto";
              if (numCols === 2 && isShorts) {
                // colIdx === 0 মানে বাম কলাম (ডানে চাপবে), colIdx === 1 মানে ডান কলাম (বামে চাপবে)
                alignmentClass = colIdx === 0 ? "ml-auto mr-0" : "mr-auto ml-0"; 
              }
              ...
              return (
                <div 
                  key={item.id || idx} 
                  className={`relative w-full overflow-hidden rounded-lg cursor-pointer group bg-neutral-900 border border-neutral-800 transition-all duration-300 ${
                    isShorts ? `max-w-[280px] sm:max-w-[320px] ${alignmentClass}` : ""
                  }`} 
                  style={{ paddingTop: aspectRatio }}
                >
